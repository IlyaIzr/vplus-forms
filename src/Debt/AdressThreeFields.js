...
export const AdressThreeFields = ({ state, setState }) => {
  const options = remaker(countries)

  const addField = () => {
    const mutable = [...state.value, { country: '', city: '', adress: '' }]
      ...
      {state.value && Boolean(state.value.length) && state.value.map((field, index) => {
        ...
        return (
          <div className="ui segment three fields" key={index+'adress'}>
            ...
            <StringField label="@(Город)" name="city"
              isRequired={false}
              value={field.city} onChange={onChange}
            />

            <StringField label="@(Адрес)" name="adress"
              isRequired={false}
              value={field.adress} onChange={onChange}
            />
...
const remaker = arr => {
  const result = []
  arr.map((obj, index) => {
    result.push({ label: obj.name, value: obj.name })
    // wri.push(`{ label: "${obj.name}", value: "${obj.name}" }`)
  })
  return result
}